...
impl RedisUtils {
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn set_string<K, V>(&self, key: K, value: V, expiry: Option<u64>) -> Result<(), AppError>
    where
        ...
        K: redis::ToRedisArgs + Send + Sync,
    {
        ...
        match result {
            ...
            Err(e) => Err(AppError::Internal(anyhow::anyhow!("Redis get failed: {}", e))),
        }
    }
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn set_json<K, V>(&self, key: K, value: &V, expiry: Option<u64>) -> Result<(), AppError>
    where
        K: redis::ToRedisArgs + Send + Sync,
        V: Serialize,
    {
        ...
        K: redis::ToRedisArgs + Send + Sync,
    {
        ...
        let mut conn = self.manager.connection().clone();
        let result: bool = conn.expire(key, seconds as i64)
            .await
        ...
        V: redis::ToRedisArgs + Send + Sync,
    {
        ...
        let mut conn = self.manager.connection().clone();
        let length: u32 = conn.lpush(key, value)
            .await
        ...
        V: redis::ToRedisArgs + Send + Sync,
    {
        ...
        let mut conn = self.manager.connection().clone();
        let length: u32 = conn.rpush(key, value)
            .await
        ...
        K: redis::ToRedisArgs + Send + Sync,
    {
        ...
        match result {
            Ok(value) => Ok(Some(value)),
            Err(e) if e.kind() == redis::ErrorKind::TypeError => Ok(None),
            Err(e) => Err(AppError::Internal(anyhow::anyhow!("Redis lpop failed: {}", e))),
        }
    }
        ...
        K: redis::ToRedisArgs + Send + Sync,
    {
        ...
        match result {
            Ok(value) => Ok(Some(value)),
            Err(e) if e.kind() == redis::ErrorKind::TypeError => Ok(None),
            Err(e) => Err(AppError::Internal(anyhow::anyhow!("Redis rpop failed: {}", e))),
        }
    }
        ...
        V: redis::ToRedisArgs + Send + Sync,
    {
        ...
        let mut conn = self.manager.connection().clone();
        let added: u32 = conn.sadd(key, member)
            .await
        ...
        V: redis::ToRedisArgs + Send + Sync,
    {
        ...
        let mut conn = self.manager.connection().clone();
        let removed: u32 = conn.srem(key, member)
            .await
