...
impl CacheHelper {
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn cache_user<T>(&self, user_id: u32, user_data: &T, ttl_seconds: Option<u64>) -> Result<()>
    where
        T: Serialize,
    {
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn set_session<T>(&self, session_id: &str, session_data: &T, ttl_seconds: u64) -> Result<()>
    where
        T: Serialize,
    {
        let key = format!("{}{}", cache_keys::SESSION_PREFIX, session_id);
        self.redis_utils.set_json(key, session_data, Some(ttl_seconds)).await
    }
    ...
    pub async fn rate_limit(&self, identifier: &str, limit: i64, window_seconds: u64) -> Result<bool> {
    ...
    pub async fn get_rate_limit_count(&self, identifier: &str) -> Result<i64> {
        ...
        if let Some(count_str) = self.redis_utils.get_string(&key).await? {
            count_str.parse::<i64>()
                .map_err(|e| AppError::Internal(anyhow::anyhow!("Failed to parse rate limit count: {}", e)))
        } else {
            Ok(0)
        }
    }
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn set_verification_code(&self, identifier: &str, code: &str, ttl_seconds: u64) -> Result<()> {
        let key = format!("{}{}", cache_keys::VERIFICATION_PREFIX, identifier);
        self.redis_utils.set_string(key, code, Some(ttl_seconds)).await
    }
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn add_to_list<T>(&self, list_key: &str, item: &T, max_length: Option<u32>) -> Result<()>
    where
        T: Serialize,
    {
        ...
        if let Some(max_len) = max_length {
            ...
            let _: () = conn.ltrim(list_key, 0, (max_len as isize) - 1)
                .await
        ...
        T: for<'de> Deserialize<'de>,
    {
        ...
        let mut conn = self.redis_utils.manager.connection().clone();
        let items: Vec<String> = conn.lrange(list_key, start as isize, end as isize)
            .await
            .map_err(|e| AppError::Internal(anyhow::anyhow!("Redis lrange failed: {}", e)))?;
        
        let mut result = Vec::new();
        for item_str in items {
            let item: T = serde_json::from_str(item_str.as_str())
    ...
    /// 
    /// 返回 `Result<(), AppError>`
    pub async fn batch_set(&self, items: Vec<(String, String)>, ttl_seconds: Option<u64>) -> Result<()> {
        use redis::AsyncCommands;
        ...
        for (key, value) in items {
            if let Some(seconds) = ttl_seconds {
                let _: () = conn.set_ex(key, value, seconds)
                    .await
                    .map_err(|e| AppError::Internal(anyhow::anyhow!("Redis batch set_ex failed: {}", e)))?;
            } else {
                let _: () = conn.set(key, value)
                    .await
                    .map_err(|e| AppError::Internal(anyhow::anyhow!("Redis batch set failed: {}", e)))?;
            }
        }
    ...
    pub async fn health_check(&self) -> Result<bool> {
        ...
        match self.redis_utils.set_string(test_key, test_value, Some(1)).await {
            Ok(_) => {
                // 尝试删除测试键
